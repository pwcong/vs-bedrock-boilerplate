...
import { findFirstIdxMonotonousOrArrLen } from '../arrays-find';
...
export function sortedDiff<T>(
  ...
  function pushSplice(start: number, deleteCount: number, toInsert: T[]): void {
    ...
    if (latest && (latest as any).start + latest.deleteCount === start) {
      latest.deleteCount += deleteCount;
      latest.toInsert.push(...toInsert);
    } else {
      result.push({ start, deleteCount, toInsert } as any);
    }
  }
...
export function coalesce<T>(array: ReadonlyArray<T | undefined | null>): T[] {
  return array.filter((e): e is T => Boolean(e));
}
