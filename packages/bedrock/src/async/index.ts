...
export function isThenable<T>(obj: unknown): obj is Promise<T> {
  return Boolean(obj) && typeof (obj as Promise<T>).then === 'function';
}
...
export class Delayer<T> implements IDisposable {
  ...
  isTriggered(): boolean {
    return Boolean(this.deferred?.isTriggered());
  }
 ...
 */
export type AsyncIterableExecutor<T> = (
  emitter: AsyncIterableEmitter<T>,
) => void | Promise<void>;
...
export class AsyncIterableObject<T> implements AsyncIterable<T> {
  ...
  public static coalesce<T>(
    ...
    return AsyncIterableObject.filter(iterable, item =>
      Boolean(item),
    ) as AsyncIterableObject<T>;
  }
